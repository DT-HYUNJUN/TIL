...
컴포넌트는 props로 데이터를 입력받아 화면에 출력한다.

### 3. Virtual DOM

리액트에서는 가상 돔 (virtual DOM)을 사용하여 렌더링을 실행한다.

가상 돔이란 실제 DOM을 조작하는 방식이 아닌, 실제 DOM을 모방한 가상의 DOM을 만들고 원래의 DOM과 비교하여 변경된 부분을 리렌더링 하는 방식을 말한다.

### 4. React Hook

React Hook은 클래스형 컴포넌트에서 사용하던 코드들을 작성할 필요 없이 함수형 컴포넌트에서 다양한 기능을 사용할 수 있게끔 만들어진 라이브러리로 함수형 컴포넌트에서만 사용 가능하고 몇가지 사용 규칙이 있다.

React Hook은 최상위에서만 사용할 수 있고 리액트 함수 내에서만 사용할 수 있다.

자주 사용하는 hooks는 useState, useEffect, useRef, useMemo, useCallback 등이 있다.
...
HTTP URI를 통해 자원을 명시하고 HTTP Method를 통해 해당 자원에 대한 CRUD 작업을 적용하는 것을 말한다.
...
1. URI는 동사보다는 `명사`를, 대문자보다는 `소문자`를 사용한다.
